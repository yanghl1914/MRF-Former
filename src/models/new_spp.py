...
def spatial_pyramid_pool(previous_conv, num_sample, previous_conv_size, out_pool_size):
    for i in range(len(out_pool_size)):
        ...
        print("psize:",previous_conv.size())
        x = maxpool(previous_conv)
        if (i == 0):
            spp = x
            print("spp size:",spp.size())
            spp1 = x.view(num_sample,-1)
            print("spp11 size:",spp1.size())
        else:
            print("xsize:",x.size())
            # spp = torch.cat((spp, F.interpolate(x, size=(spp.shape[-1],spp.shape[-2]), mode='bilinear', align_corners=False)), 1)
            spp = spp + F.interpolate(x, size=(spp.shape[-1], spp.shape[-2]), mode='bilinear', align_corners=False)
            print("spp1 size:",spp.size())
    return spp.flatten(2)
...
class SPP_3D_NET(nn.Module):
    ...
    def __init__(self, input_nc=3, ndf=64, gpu_ids=[0, 1, 2, 3, 4]):
        ...
        # self.output_num = [64]
        self.output_num = [64, 32, 16, 8, 4, 2, 1]
        # self.weights = nn.Parameter(torch.ones(len(self.output_num), dtype=torch.float32), requires_grad=True)
    ...
    def forward(self, x):
        ...
        spp = spatial_pyramid_pool(x, B, [int(x.size(2)), int(x.size(3))], self.output_num)
        # print(spp.shape)
        # fc1 = self.fc1(spp)
        # fc2 = self.fc2(fc1)
        x = F.relu(spp)
